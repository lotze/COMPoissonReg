	...
	data = NULL, beta.init = NULL, gamma.init = NULL, zeta.init = NULL, ...)
{
	...
	n = length(y)
	if (n != nrow(S)) {
		msg = sprintf(paste("formula.nu yielded S matrix with %d rows",
			"which did not match %d observations. Try using `data` argument",
			"to pass your data"),
			nrow(S), n)
		stop(msg)
	}
	...
	if (!is.null(formula.p)) {
		...
		if (n != nrow(W)) {
			msg = sprintf(paste("formula.p yielded W matrix with %d rows",
				"which did not match %d observations. Try using `data` argument",
				"to pass your data"),
				nrow(W), n)
			stop(msg)
		...
		res$opt.method = getOption("COMPoissonReg.optim.method", default = 'L-BFGS-B')
